/* 
   ...
   Project #4
   Due: 10/13/19
   Description: This file is the class declaration file for the Player class.
*/
...
class Player
{
    public:
        ...
        //overload the << operator to display cards in player's hand (or ____)
        friend ostream& operator << (ostream&, const Player&);
        
    private:
        string name;                //the player's name
        int score;                  //the player's score
        Card hand[Max_Cards];       //array holding the cards currently in the players hand
        bool hasPlayed[Max_Cards];  //hasPlayed[i] indicates that hand[i] (i.e., ith card in player's ahnd has been played)
